  ...
  try {
    for (;;) {
      if (!budget.canSpend() || budget.throttleImminent()) {
        return { done: false, cursor, stats: { seen, skipped, ...budget.stats } };
      }
      ...
      if (!cursor) {
        const stats = { seen, skipped, ...budget.stats };
    ...
    await finishRun(state.runId, 'failed', { seen, skipped, ...budget.stats }, message);
    throw error;
  }
}
