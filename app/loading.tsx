  ...
  return (
    <div className="animate-pulse space-y-10" aria-busy="true" aria-label="Loading">
      <div className="space-y-3">
        <div className="h-7 w-48 rounded-md bg-[--color-paper-sunk]" />
        <div className="h-4 w-80 max-w-full rounded bg-[--color-paper-sunk]" />
      </div>

      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {[0, 1, 2].map((i) => (
          <div key={i} className="h-52 rounded-[--radius-note] bg-[--color-paper-sunk]" />
        ))}
      </div>

      <div className="space-y-3">
        <div className="h-24 rounded-xl bg-[--color-paper-sunk]" />
        <div className="h-24 rounded-xl bg-[--color-paper-sunk]" />
      </div>
    </div>
  );
